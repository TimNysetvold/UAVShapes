function [ conflictindex ] = findConflicts(distances,dronearray)
...
%this function makes me cry
    ...
    for i=1:size(dronearray,1)
        
        collisiondistance=dronearray(i,16); %J16 is the separation standard for the drone being checked
        ...
        allconflictspriority=dronearray(index,15);
        %stubarray holds the priorities of drones in conflict with the
        %current i drone.
        
        highpriorityconflictsindexofindexes=find(allconflictspriority<=dronearray(i,15));
        ...
        if(~isempty(highpriorityconflictsindex))
            ...
            %value than j does (higher is worse), i must avoid j. 
                ...
                if (dronearray(i,17)==0 && dronearray(highpriorityconflictsindex(j),17)==0)
