...
for currentIteration=1:iterations
    ...
    intermediatearrayNF=intermediatearray0s;  %We reset the list of intermediate steps.
    intermediatearrayF=intermediatearray0s;
...
%     graphDrones(totaldronearray,patharray,newcrashes,linecapture,baselength,currentIteration)
    ...
    thisconflictindexNF=findConflicts(distances_xyz,totaldronearray);
    ...
    for i=1:sum(numdrones)
       ...
       if(~isempty(thisconflictindexNF))
           ...
           thisconflictindexF = conflictReducer(thisconflictindexNF,visibleDrones);
       else
           thisconflictindexF = thisconflictindexNF;
       end
       
%        if ~all(all(thisconflictindexF == thisconflictindexNF))
%            disp 'error'
%        end
       if currentIteration == 9683
           disp 'time of interest'
       end
              
       if(~isempty(thisconflictindexF)) %if there is a conflict, find the entry to which it corresponds
           [row,~]=find(thisconflictindexF(:,1)==i);
            curconflictsF=thisconflictindexF(row,2);
            ...
            if (~isempty(curconflictsF))
            %If there is a conflict, handle it within this statement.
                intermediatearrayF(i,1:4) = PotentialFieldsZActive(i,totaldronearray,curconflictsF,patharray);
            end
            
       end
       
       if(~isempty(thisconflictindexNF)) %if there is a conflict, find the entry to which it corresponds
           [row,~]=find(thisconflictindexNF(:,1)==i);
            curconflictsNF=thisconflictindexNF(row,2);
            
            %Now, curconflicts has the index of all drones infringing on the
            %current drone. If none of the conflicts in the index involve
            %it, then this array will be empty and we'll ignore it.
            if (~isempty(curconflictsNF))
            %If there is a conflict, handle it within this statement.
                intermediatearrayNF(i,1:4) = PotentialFieldsZActive(i,totaldronearray,curconflictsNF,patharray);
            end
            
       end
       
%        if any(size(curconflictsNF) ~= size(curconflictsF))
%            disp error
%        end
%        
%        try
%            if ~all(all(curconflictsNF == curconflictsF))
%                 disp 'error'
%            end
%        catch ME
%            disp faild
...
%        end
       ...
       if ~all(all(intermediatearrayNF == intermediatearrayF))
           disp 'error'
       end
       
       if (currentIteration == 50) && (i == sum(numdrones))
        disp 'pause'
        end
       ...
       [intermediatearrayNF(i,:),patharray{i,1}]=getObjective(totaldronearray(i,:),intermediatearrayNF(i,:),patharray{i,1},baselength,layerMat);     
       [intermediatearrayF(i,:),patharray{i,1}] =getObjective(totaldronearray(i,:),intermediatearrayNF(i,:), patharray{i,1},baselength,layerMat);
       %UAV moves
       totaldronearraytemp = totaldronearray;
       totaldronearrayhistoric = totaldronearray;
       totaldronearraydeterminism = totaldronearray;
       
       totaldronearray(i,:)    = updatePosition(totaldronearrayhistoric(i,:),intermediatearrayNF(i,1:4),patharray{i,1});
       totaldronearraydeterminism(i,:) = updatePosition(totaldronearrayhistoric(i,:),intermediatearrayNF(i,1:4),patharray{i,1});
       totaldronearraytemp(i,:) = updatePosition(totaldronearrayhistoric(i,:),intermediatearrayF(i,1:4), patharray{i,1});
        
        
       if ~all(all(totaldronearray == totaldronearraytemp))
           disp 'error'
       end
    end
