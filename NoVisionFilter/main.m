...
for currentIteration=1:iterations
    ...
    thisconflictindex=findConflicts(distances_xyz,totaldronearray);
...
%        visibleDroneArray = totaldronearray(visibleDrones(:),:); 
%        visibleDistanceArray = distances_xyz(visibleDrones(:),visibleDrones(:));
             
%        thisconflictindex=findConflicts(visibleDistanceArray,visibleDroneArray);
       ...
       if(~isempty(thisconflictindex)) %if there is a conflict, find the entry to which it corresponds
           [row,~]=find(thisconflictindex(:,1)==i);
            ...
            %If there is a conflict, handle it within this statement.
                %intermediatearray(i,1:4) = PotentialFieldsZActive(i,visibleDroneArray,curconflicts,patharray);
